...
# given a value to the function say -> partion_list(5)
# we simply rearrange the linked list
# where the first half are values less than 5
# and the second half we have values higher than 5
# and the values higher than 5 are also added
# so a list like: [8, 3, 4, 1, 6, 4, 9]
# becomes: [3, 4, 1, 4] and [8, 6, 9]
    ...
    def partition_list(self, x: int) -> None:
        ...
        while temp:
            if temp.value < x:
                ...
                prev_a.next = temp
                # previous node is the current one
                prev_a = temp
            else:
                prev_b.next = temp
                prev_b = temp
