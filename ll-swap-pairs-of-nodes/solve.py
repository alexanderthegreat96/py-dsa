...
#  so we're just swapping nodes if there is an even number of them
# if there an odd number, we just leave it as it is
...
class LinkedList:
    ...
    def swap_pairs(self):
        ...
        # dummy node before everything
        # it points to the head of the list

        dummy = Node(0)
        dummy.next = self.head

        # prev is initially set to head as well, through dummy node
        prev = dummy

        # so we're jumping over 2 elements
        # hence why next.next
        while prev.next and prev.next.next:
            first = prev.next  # points to what the prev pointed to
            second = prev.next.next  # points to the second node (first + 1)

            # prev now points to second
            prev.next = second
            # first points to what second was pointing to
